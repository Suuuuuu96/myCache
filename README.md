...
single flight机制可以解决缓存击穿问题。<br>
GroupCall 是 singleflight 的主数据结构，管理不同 key 的请求(call)。有两个方法：<br>
...
* fn是一个能返回key对应值的函数。fn函数的具体内容：使用 PickPeer() 方法选择节点，若非本机节点，则调用 getFromPeer() 从远程获取。若是本机节点或远程获取失败，则回退到 getLocally()。<br><br>
例子：<br>
