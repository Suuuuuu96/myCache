...
func (m *Map) Add(keys ...string) {
	...
	sort.Ints(m.keys) // 需要对环上的哈希值进行排序，后面才能进行二分查找。
...
/*
func NewMap(hash0 Hash,replicas0 int) *Map {
	ans:=&Map{
		hash:hash0,
		replicas:replicas0,
		hashMap:make(map[int]string),
...
func (m *Map)Add(keys ...string)  {
	for _,key:=range keys{
		for i:=0;i<m.replicas;i++{
			temp:=m.hash([]byte(key+strconv.Itoa(i)))
			m.keys=append(m.keys,int(temp))
			m.hashMap[int(temp)]=key
		}
	}
	sort.Ints(m.keys)
...
func (m *Map)Get(data string) string {
	key:=m.hash([]byte(data))

	index:=sort.Search(len(m.keys), func(i int) bool {
		return int(key)>=m.keys[i]
	})
	val:=m.hashMap[m.keys[index%len(m.keys)]]
	return val
}
*/